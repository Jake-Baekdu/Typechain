...
const getHahsfroBlock = (aBlock : Block) : string => Block.calculateBlockhash(aBlock.index, aBlock.previousHash, aBlock.timestamp, aBlock.data)
const isBlockValid = (candidateBlock : Block, previousBlock: Block) : boolean => {
 if(Block.validateStructure(candidateBlock)){
    ...
    return false;
 }  else if(getHahsfroBlock(candidateBlock) !== candidateBlock.hash){
    return false;
 } else{
     return true;
 }
